...
export const TRUSTED_DOMAINS = [
  // المواقع والمنصات الحكومية السعودية
  'gov.sa',
  'sa.gov.sa',
  'my.gov.sa',
  'absher.sa',
...
export function buildStaticEvidence(normalizedUrl, canonical) {
  ...
  const suspiciousTld = SUSPICIOUS_TLDS.some(
    (t) => canonical.domain.endsWith(t)
  );
  ...
  const fullPath =
    (normalizedUrl.hostname + normalizedUrl.pathname).toLowerCase();
  ...
  const lookalike =
    !trusted &&
    BRAND_LOOKALIKES.some(
      (b) => canonical.domain.includes(b)
    );
